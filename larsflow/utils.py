...
def evaluateAldp(model, test_data, n_samples=1000, n_batches=1000,
    ...
    nbins = 64
    eps = 1e-10
    hist_test = np.histogram2d(phi_d, psi_d, nbins, range=[[-np.pi, np.pi], [-np.pi, np.pi]])[0]
    hist_gen = np.histogram2d(phi, psi, nbins, range=[[-np.pi, np.pi], [-np.pi, np.pi]])[0]
    kld_ram = np.sum(hist_test * np.log(hist_test + eps) / np.log(hist_gen + eps)) / len(phi)
